...
const sendEmail = async (user , callback: Callback) => {
  ...
  .then(async (data) => {
    ...
    await dynamodb.update(userUpdated).promise().then(async() => {
      await sendWelcomeEmail({...data.Item, linkToPdf: user.linkToPdf} as CreatedUser)
...
const uploadPdf = async (event: APIGatewayProxyEvent, context: Context, callback: Callback) => {
