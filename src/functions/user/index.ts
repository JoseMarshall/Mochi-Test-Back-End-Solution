...
import { removeEspecialChars, isValideEmail, sanitizeHtml } from "../../helpers"
...
const submitUser = async (user: CreatedUser) => {
  ...
  return dynamodb.put(newUser).promise().then(()=>user)

};

const generateUsernameSuggestions = ({ firstName, lastName, username, }: FormData) => {
...
const validateFormData = (data: FormData) => {
  return new Promise<string[]>(async (resolve, reject) => {
    ...
    if (typeof firstName !== 'string' ||
      ...
      !isValideEmail(email)) {
      reject(new Error(`Verifique se o campo 'firstName' e 'lastName' sao do tipo 'string' e se o email é válido`))
      return;
    }
    else {
...
const createUser = (event: APIGatewayProxyEvent, context: Context, callback: Callback) => {
